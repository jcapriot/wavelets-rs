...
use quote::quote;
use syn::{
    DeriveInput, Ident, Result, Token, Type, parse::{Parse, ParseStream}, parse_macro_input
};
...
impl Parse for LiftingScheme<f64>{
    fn parse(input: ParseStream) -> Result<Self>{
        ...
        let steps: Vec<LiftingStep<f64>> = 
            input.parse_terminated(LiftingStep::<f64>::parse, Token![,])?
            .into_iter()
            .collect();
        Ok(Self{name, steps})
    }
...
fn expand_lifting_step(step: &LiftingStep<f64>, direction: LiftingDirection) -> proc_macro2::TokenStream {
    ...
    match step {
        LiftingStep::UpdateD { offset, coefs } | LiftingStep::UpdateS { offset, coefs } => {
            ...
            if *offset < 0 {
                let accumulators = coefs.iter().enumerate()
                    .filter(|(_, v)| **v != 0.0).map(|(j, _)|{
