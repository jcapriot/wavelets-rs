use ndarray::{
    ArrayView, ArrayView1, StrideShape, Ix1, Dimension, ArrayRef, Axis, RemoveAxis, ShapeBuilder,
    ArrayViewMut, ArrayViewMut1
};
...
impl<'a, A, D: Dimension,  const N: usize> LaneChunksExact<'a, A, D, N>{
    fn get_chunk(&self, ind: usize) -> [ArrayView1<'a, A>; N]{
        ...
        std::array::from_fn(|_| {
            ...
            //advance inds;
            for(i_d, n_d) in inds.slice_mut().iter_mut()
            ...
            unsafe{
                    ArrayView1::from_shape_ptr(self.inner_shape, ptr)
            }
        })
    }
}
...
impl<'a, A, D: Dimension,  const N: usize> LaneChunksExactMut<'a, A, D, N>{
    fn get_chunk(&mut self, ind: usize) -> [ArrayViewMut1<'a, A>; N]{
        ...
        std::array::from_fn(|_| {
            ...
            unsafe{
                    ArrayViewMut1::from_shape_ptr(self.inner_shape, ptr)
            }
        })
    }
}
...
pub trait LaneChunksExactIterator<A, D: Dimension>{
    fn lane_chunks_iter<const N: usize>(&self, axis: Axis)-> (
...
impl<A, D:Dimension + RemoveAxis> LaneChunksExactIterator<A, D> for ArrayRef<A, D>{
    fn lane_chunks_iter<const N: usize>(&self, axis: Axis)-> (
        LaneChunksExact<'_, A, D::Smaller, N>, LaneChunksExact<'_, A, D::Smaller, 1>
    ) {
        let shape = (self.len_of(axis), )
            .strides((usize::try_from(self.stride_of(axis)).expect("Cannot iterate lanes with negative strides."),)
        );
        let view = self.view();
        ...
        (
            LaneChunksExact{ base: sub_arr.clone(), inner_shape: shape, bounds: (0, n_chunks) },
            LaneChunksExact{ base: sub_arr, inner_shape: shape, bounds: (n_chunks * N, end) }
        )
...
pub trait LaneChunksExactIteratorMut<A, D: Dimension>{
    fn lane_chunks_iter_mut<const N: usize>(&mut self, axis: Axis)-> (
        LaneChunksExactMut<'_, A, D::Smaller, N>, LaneChunksExactMut<'_, A, D::Smaller, 1>
    );
}
...
impl<A, D:Dimension + RemoveAxis> LaneChunksExactIteratorMut<A, D> for ArrayRef<A, D>{
    fn lane_chunks_iter_mut<const N: usize>(&mut self, axis: Axis)-> (
        ...
        strides.slice_mut().iter_mut()
            .zip(sub_arr.strides().iter())
            .for_each(|(sc, s)| {
                *sc = usize::try_from(*s).expect("Cannot handle negative strides for mutable lane chunk iterator.")}
            );
        ...
        (
            LaneChunksExactMut{ base: sub_arr, inner_shape: shape, bounds: (0, n_chunks) },
            LaneChunksExactMut{ base: sub_arr_clone, inner_shape: shape, bounds: (rem_start, end) },
        )
    }
}
...
pub mod par{
    ...
    impl<'a, const N:usize, A: Sync, D:Dimension> ParallelIterator for ParLaneChunksExact<'a, A, D, N>{
        type Item = [ArrayView1<'a, A>; N];
    ...
    impl<'a, const N:usize, A: Sync, D:Dimension> IndexedParallelIterator for ParLaneChunksExact<'a, A, D, N>{
    ...
    impl<'a, const N:usize, A: Sync + Send, D:Dimension> ParallelIterator for ParLaneChunksExactMut<'a, A, D, N>{
        type Item = [ArrayViewMut1<'a, A>; N];
        fn drive_unindexed<C>(self, consumer: C) -> C::Result
        where
            C: UnindexedConsumer<Self::Item>
    ...
    impl<'a, const N:usize, A: Sync + Send, D:Dimension> IndexedParallelIterator for ParLaneChunksExactMut<'a, A, D, N>{
        fn drive<C>(self, consumer: C) -> C::Result
        where
            C: Consumer<Self::Item>,
        {
    ...
    impl<'a, const N:usize, A: Sync + Send, D:Dimension> Producer for LaneChunksExactMutProducer<'a, A, D, N>{
        type Item = [ArrayViewMut1<'a, A>; N];
        ...
        fn split_at(mut self, index: usize) -> (Self, Self) {
            ...
            // which is gauranteed not to overlap with the chunk iterator
            unsafe{
                base_clone = ArrayViewMut::from_shape_ptr(self.outer_shape.clone(), self.base.as_mut_ptr());
            }
            (
                Self{
                    base: base_clone,
    ...
    pub trait ParLaneChunksExactIterator<A, D: Dimension>{
        fn lane_chunks_par_iter<const N: usize>(&self, axis: Axis)-> (
            ParLaneChunksExact<'_, A, D::Smaller, N>, ParLaneChunksExact<'_, A, D::Smaller, 1>
        );
    }
    ...
    impl<A, D:Dimension + RemoveAxis> ParLaneChunksExactIterator<A, D> for ArrayRef<A, D>{
        fn lane_chunks_par_iter<const N: usize>(&self, axis: Axis)-> (
            ...
            (
                ParLaneChunksExact { base: sub_arr.clone(), inner_shape: shape, bounds: (0, n_chunks) },
                ParLaneChunksExact { base: sub_arr, inner_shape: shape, bounds: (n_chunks * N, end) }
            )
        }
    }

    pub trait ParLaneChunksExactIteratorMut<A, D: Dimension>{
        fn lane_chunks_par_iter_mut<const N: usize>(&mut self, axis: Axis)-> (
    ...
    impl<A, D:Dimension + RemoveAxis> ParLaneChunksExactIteratorMut<A, D> for ArrayRef<A, D>{
        fn lane_chunks_par_iter_mut<const N: usize>(&mut self, axis: Axis)-> (
            ...
            // which is gauranteed not to overlap with the chunk iterator
            unsafe{
                sub_arr_clone = ArrayViewMut::from_shape_ptr(sub_shape.clone(), sub_arr.as_mut_ptr());
            }
            let n_chunks = sub_arr.len() / N;
            let end = sub_arr.len();

            (
                ParLaneChunksExactMut { base: sub_arr_clone, outer_shape: sub_shape.clone(), inner_shape: shape, bounds: (0, n_chunks) },
                ParLaneChunksExactMut { base: sub_arr, outer_shape: sub_shape, inner_shape: shape, bounds: (n_chunks * N, end) }
            )
        }
    }
}