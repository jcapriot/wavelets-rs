...
pub fn dwt_forward<T: Num + Clone + std::fmt::Debug, const N: usize, BC: BoundaryExtension>(g: &[T; N], h: &[T; N], x: &[T], s: &mut [T], d: &mut[T], _bc: &BC){

    let (nx, ns, nd) = (x.len(), s.len(), d.len());
    assert!(ns == nd || ns == nd + 1, "'d.len()' must be equal to or 1 less than 's.len()'");
    assert_eq!(nx, ns + nd, "'s.len() + d.len()' must be equal to 'x.len()'");
    ...
    sd_iter.by_ref().take(n_front).for_each(|(i, (s, d))|{
        let ix = 2 * i - offset;
        (*s, *d) = (0..N as isize).zip(gh_iter.clone()).map(|(j, (g,  h))|{
            let xo = BC::get_bc(x, ix + j);
            (g.clone() * xo.clone(), h.clone()  * xo)
        }).fold(
            (T::zero(), T::zero()),
            |(s_s, d_s), (s, d)|(s + s_s, d + d_s)
        );
    });

    let first_x = offset as usize % 2;

    sd_iter.by_ref().zip(x[first_x..].windows(N).step_by(2))
        .for_each(|((_i, (s, d)), x)|
        {
            (*s, *d) = gh_iter.clone().zip(x)
                .map(
                    |((g, h), xi)| (g.clone() * xi.clone(), h.clone() * xi.clone())
                )
                .fold(
                    (T::zero(), T::zero()),
                    |(s_s, d_s), (s, d)|(s + s_s, d + d_s)
    ...
    if nd < ns {
        ...
        s[nd] = (0..N as isize).zip(gh_iter.clone()).map(|(j, (g,  _))|{
            let xo = BC::get_bc(x, ix + j);
            g.clone() * xo.clone()
        }).reduce(|acc, v| acc + v ).unwrap_or(T::zero());
    }
