...
where
    T: Transformable + Zero + ChunkWidth<T, N> + Sized,
            ...
            match first {
                ...
                false => {
                    ...
                    chunks.for_each_init(
                        ...
                        |(s, d), chunk| {
                            // copy (and deinterleave) strided chunks into the local storage
                            deinterleave_strided_chunk(chunk.clone().into(), s, d);
                            s.chunks_exact_mut(n_s)
                                .zip(d.chunks_exact_mut(n_d))
                                .for_each(|(s, d)| {
                                    func(s, d);
                            ...
                            // clone local storage to the output
                            stack_to_strided_chunk(s, d, chunk);
                        },
                    );
                    rem.for_each_init(
                        || {
                        ...
                        |(s, d), slc| {
                            // copy strided slice into local dimension storage
                            deinterleave_strided(slc.clone().into(), s, d);
                            func(s, d);
                            // copy local back to output strided slice
                            stack_to_strided(s, d, slc);
                        },
                    );
                }
            }
                ...
                false => {
                    ...
                    if chunks.len() > 0 {
                        ...
                        chunks.for_each(|chunk| {
                            // copy (and deinterleave) strided chunks into the local storage
                            deinterleave_strided_chunk(chunk.clone().into(), &mut s, &mut d);
                            s.chunks_exact_mut(n_s)
                                .zip(d.chunks_exact_mut(n_d))
                                .for_each(|(s, d)| {
                                    func(s, d);
                    ...
                    if rem.len() > 0 {
                        ...
                        rem.for_each(|slc| {
                            // copy strided slice into local dimension storage
                            deinterleave_strided(slc.clone().into(), &mut s, &mut d);
                            func(&mut s, &mut d);
                            // copy local back to output strided slice
                            stack_to_strided(&s, &d, slc);
                        });
    ...
    T::ScalarType: From<f64>,
{
    ...
    for lvl in (0..level).rev() {
        ...
        for &ax in axes {
            ...
            let (chunks, rem) = output.par_iter_lane_chunks_mut_strided::<N>(shape, stride_out, ax);
            if chunks.len() > 0 {
                chunks.for_each_init(
                    || {
                        let s = vec![T::zero(); n_s * N];
...
//     #[rstest]
//     fn test_roundtrip(#[values(12, 13)] n: usize, #[values(1, 2, 3, 4)] dim: usize) {
//         let shape = vec![n; dim];
//         let stride = stride_from_shape(&shape)
//             .into_iter()
//             .map(|i| i as isize)
