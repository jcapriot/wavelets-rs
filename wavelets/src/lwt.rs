...
pub fn stack_to_strided<'a, T: Clone>(first: &[T], second: &[T], out: &'a mut MutStridedSlice<'a, T>){
...
pub fn interleave<T: Clone>(evens: &[T], odds: &[T], x: &mut [T]){
    ...
    rem.iter_mut().zip(ev_iter).for_each(|(x, ev)| *x = ev.clone());
}

const N: usize = 4;

pub fn deinterleave_strided_chunk<T: Clone, const N: usize>(x: &ChunkStridedSlice<T, N>, evens: &mut [T], odds: &mut [T]){
    assert_ne!(N, 0);
...
pub fn stack_to_strided_chunk<'a, T: Clone, const N: usize>(first: &[T], second: &[T], out: &'a mut MutChunkStridedSlice<'a, T, N>){
    assert_ne!(N, 0);
...
pub fn general_nd_forward<T>(func: fn(&mut[T], &mut[T]), input: &[T], output: &mut[T], shape: &[usize], axes: &[usize])
where
...
pub mod parallel{
        ...
        T: Num + NumAssignOps + Clone + From<f64> + Sync + Send
    {
        ...
        for &ax in axes{
            ...
            if iter_in_chunks.len() > 0 {
                iter_in_chunks.zip(iter_out_chunks).with_min_len(min_len).for_each_init(
